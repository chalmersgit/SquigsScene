...
def setColour(i, objectName, r, g, b, shaderType):
    colourName = 'colour'+str(i)
    sg = 'SG'+str(i)
    cmds.select(planeNameVar)
    sh = cmds.shadingNode(shaderType, asShader=True,  name=colourName)
    cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name=sg)
    cmds.connectAttr(colourName+'.outColor', sg+'.surfaceShader', force=True)
    cmds.setAttr(colourName+".color", r, g, b, type='double3')
    #cmds.sets(name, edit=True, forceElement=[colourName, sg])
...
def createHoneyPiece(n, b):
    ...
    sh = cmds.shadingNode('phong', asShader=True,  name=colourName)
    cmds.sets(renderable=True, noSurfaceShader=True, empty=True, name=sg)
    cmds.connectAttr(colourName+'.outColor', sg+'.surfaceShader', force=True)
    cmds.setAttr(colourName+".color", 0.353, 0.31717, 0.291578, type='double3')
    #cmds.sets(name, edit=True, forceElement=[colourName, sg])
    cmds.select(planeName)
    cmds.hyperShade(assign=sh)
    '''
    ...
    #delete if we want holes in our plane
    if(b):
        cmds.delete();
...
for i in range(1):
    ...
    if(i > 0):
        cmds.move(random.randint(a, b), random.randint(a, b), 0, planeNameVar, a=True);
    else:
        cmds.move(0, -5, 0, planeNameVar, a=True)
    createHoneySphere(pieceNumber);
    #cmds.scale(1, 1.550722, 1, a=True)
    cmds.select(cl=True);
    setColour(pieceNumber, planeNameVar, 1, 0.77, 0, 'phong')
    #cmds.scale(1, 5, 1, r=True)
    #cmds.setAttr(planeNameVar+".translateY",  random.randint(a, b)) #increases size
    pieceNumber += 1;
...
for i in range(16):
    ...
    createHoneySphere(pieceNumber);
    cmds.rotate(random.randint(1, 60), random.randint(1, 60), random.randint(1, 60), os=True, r=True);
    cmds.select(cl=True);
    #cmds.setAttr(planeNameVar+".translateY",  random.randint(a, b))
    pieceNumber += 1;

cmds.setAttr("lambert1.color", 0.353, 0.31717, 0.291578, type='double3')
'''
for i in range(1):
    planeNameVar = 'plane'+str(pieceNumber)
    cmds.select(cl=True);
...
setColour(pieceNumber, planeNameVar, 0, 0, 0, 'phong')
pieceNumber+=1
height = 5
...
cmds.scale(1, -5, 1, 'plane0', r=True)

backDrop = 'plane'+str(pieceNumber)
cmds.polyPlane(w=500, h=500, n=backDrop)
#createHoneyPiece(pieceNumber, True);
cmds.rotate(90, 0, 0, r=True, os=True )
cmds.move(0, 0, -100, r=True)
setColour(pieceNumber, backDrop, 1, 1, 1, 'lambert')
pieceNumber+=1
cmds.select(cl=True);